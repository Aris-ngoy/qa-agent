...
export function createApp(settings: RunnerSettings, startedAt = Date.now()) {
	...
	app.get("/", (c) =>
		c.json({
			name: "yoqa-runner",
			docs: "GET /health · GET /status · GET /doctor · GET /servers · GET /devices · POST /devices/connect · GET /screen · GET /screenshot/image · WS /ws/control · POST /action · /apps · /cases · /flows · /tags · /providers · /runs · /builds",
		}),
	);
