...
export function formatProviderHttpError(label: string, status: number, body: string): string {
	...
	if (
		...
		return "Groq rejected strict JSON schema mode for this model (response_format requires every key in properties). The runner already compensates with prompt JSON — retry the run; if it persists, pick a different Groq model in Settings → Provider.";
	}
