...
```bash
yoqa apps list                                                 # list all workspace apps (NAME, PREFIX)
yoqa apps get APP                                              # app details, including the current app context
yoqa apps get APP --json                                       # raw JSON (use to read the full app_context)
yoqa apps update APP '{"name": "...", "app_context": "..."}'   # update app name or context
```

`app_context` is a shared description injected into every test run — use it for credentials, screen names, and general rules the agent should always follow. Read the current value with `yoqa apps get APP` before updating, then send the full replacement text (update overwrites, it does not append).
...
```bash
yoqa cases list APP                          # list test cases (table: ID, TITLE, TAGS)
yoqa cases list APP --tag smoke              # filter by tag
yoqa cases list APP --tag smoke --tag auth   # filter by multiple tags (AND)
yoqa cases get APP 42                        # get case details: id, title, tags, flows (JSON)
yoqa cases create APP '{"title": "...", "tags": ["smoke"], "flows": [...]}'
yoqa cases update APP 42 '{"title": "New title"}'
yoqa cases delete APP 42                      # delete a test case (irreversible)
```
...
> **Before writing any inline flow** (on create or update), run `yoqa flows APP` and check if a matching reusable flow already exists. If it does — reference it by `id`. If the step will appear in multiple cases — create a reusable flow first, then reference it.
...
```bash
yoqa flows list APP                          # list reusable flows (table: ID, NAME, USES)
yoqa flows get APP <flow-id>                 # get flow details (JSON: id, name, instructions, result)
yoqa flows create APP '{"name": "...", "instructions": "...", "result": "..."}'
yoqa flows update APP <flow-id> '{"name": "...", "instructions": "...", "result": "..."}'
yoqa flows delete APP <flow-id>              # delete a reusable flow (irreversible)
```
...
You may reuse existing tags freely, but confirm with the user before attaching tags to a case — especially when it would create a new tag rather than reuse one from `yoqa tags APP`.

```bash
yoqa tags APP                      # list tags (table: TAG, USES, CASES)
yoqa tags APP --json               # raw JSON output
```
