...
import { commandPath, environmentValues, hostReport, run } from "./env-check-helpers.mjs";

const env = environmentValues(["BUNDLETOOL_JAR", "BUNDLETOOL_PATH"]);
const configuredBundletoolJar = env.BUNDLETOOL_JAR || env.BUNDLETOOL_PATH || "";
const bundletoolJarOnPath = commandPath("bundletool.jar");
const bundletoolJar = configuredBundletoolJar || bundletoolJarOnPath || "";
...
const report = {
	...
	summary: {
		requiredOk:
			java.ok && Boolean(bundletoolJar) && existsSync(bundletoolJar) && bundletoolJarVersion.ok,
		javaOk: java.ok,
		bundletoolCommandOk: bundletoolCommand.ok,
		bundletoolJarOk: Boolean(bundletoolJar) && existsSync(bundletoolJar) && bundletoolJarVersion.ok,
	},
};
