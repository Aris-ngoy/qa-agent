...
const windowsEnvironment = environmentValues(["LOCALAPPDATA", "PROGRAMFILES", "PROGRAMFILES(X86)"]);
const browserPaths = isMac
	...
	: isWindows
		? [
				pathJoin(
					windowsEnvironment.PROGRAMFILES || "C:\\Program Files",
					"Google",
					"Chrome",
					"Application",
					"chrome.exe",
				...
				pathJoin(
					windowsEnvironment["PROGRAMFILES(X86)"] || "C:\\Program Files (x86)",
					"Google",
					"Chrome",
					"Application",
					"chrome.exe",
				...
				pathJoin(
					windowsEnvironment.LOCALAPPDATA || pathJoin(home, "AppData", "Local"),
					"Google",
					"Chrome",
					"Application",
					"chrome.exe",
...
const report = {
	...
	browsers: {
		...
		paths,
		versionCommands,
		supportedBrowserFound: commands.some((candidate) => candidate.ok) || paths.length > 0,
	...
	summary: {
		requiredOk:
			appiumPrerequisitesOk && (commands.some((candidate) => candidate.ok) || paths.length > 0),
