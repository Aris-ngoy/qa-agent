...
const avdCount = avds.ok && avds.stdout ? avds.stdout.split(/\r?\n/).filter(Boolean).length : 0;
const baseToolingOk =
	java.ok &&
	javac.ok &&
...
const targetReady = (devices.ok && connectedDeviceCount > 0) || (avds.ok && avdCount > 0);
...
function defaultAndroidHome() {
	if (isWindows) {
		return pathJoin(
			env.LOCALAPPDATA || pathJoin(env.USERPROFILE || "", "AppData", "Local"),
			"Android",
			"Sdk",
		);
	}
...
function parseDeviceCount(output) {
	return output
		.split(/\r?\n/)
		.slice(1)
		.filter((line) => /\bdevice\b/.test(line)).length;
}
