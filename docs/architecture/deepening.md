...
Deferred (follow-ups): dual config store sync.
...
- **Provider vision:** optional `vision.completeObject` on each Vision-capable Provider adapter; `decideNextAction` / `groundDescription` dispatch with no kind switch — see `docs/providers/vision-complete-object.md`
...
- Desktop catalog fallback still duplicates product facts
