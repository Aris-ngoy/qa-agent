...
- **Selector Commands** include app control: `activateApp` / `terminateApp` / `restartApp`, `openNotes` (iOS), `openUrl`, `acceptAlert` / `dismissAlert` (App ID prefilled from selected app)
...
3. App control / deeplink: select any element → **Selector Commands** → `openNotes` (iOS) or `activateApp` → insert & run; paste a deeplink in Notes → tap it → `acceptAlert` if prompted; or use `openUrl` with the deeplink directly.
