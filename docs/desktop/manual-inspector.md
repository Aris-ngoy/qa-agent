...
- **System controls:** every Argent-backed control used in manual QA maps to a `yoqa action` kind — `back`, `scroll --direction`, `home`, `keyboard`, plus the existing tap/swipe/drag/input/app-lifecycle/alert set (see matrix below). The command bar exposes swipe/scroll/back/home/keyboard without needing a selection.
...
- New actions: `back` → `argent run button --button back` (edge swipe on iOS); `scroll --direction --amount` → `argent run gesture-swipe`;
  `home` → `argent run button --button home`; `keyboard [--action dismiss|enter]` → `argent run keyboard`
- Live frames via `GET /screenshot/stream` (multipart, fresh captures back-to-back) with 500ms poll fallback; `captureFrame()` uses low-scale captures + 150ms server-side coalescing
...
- `scrollDirectionSchema` / `keyboardActionSchema`; shell format/parse for `--direction`, `--amount`, `--action`
- `shellToCaseScript` — maps inspector shell → CaseScript (resolves `--id`/`--label` via optional element tree; warns on skipped steps, including `back`/`scroll`/`home`/`keyboard`)

**Desktop — Maestro-like picker**

- **Cached Select Mode:** clicks hit-test the cached cleaned tree locally (no “Reading screen…” per click). Tree warms 500ms after connect, after Insert & Run / scripts, and via **Refresh tree**. Stale cache (>3s) triggers a background refresh without blocking the first highlight.
...
- **Selector Commands** include app control: `activateApp` / `terminateApp` / `restartApp`, `backgroundApp`, `openUrl`, `acceptAlert` / `dismissAlert` (App ID prefilled from selected app); system: `back`, `scroll up/down/left/right`, `home`, `dismiss keyboard` / `press enter`, directional `drag`; and screenshots: `screenshot` / `screenshot (path)`
...
### Argent → yoqa command matrix

| Argent tool | `yoqa action` kind | Inspector | Notes |
|---|---|---|---|
| `gesture-tap` (+ `gesture-custom` hold) | `tap` (+ `--double`, `--duration`) | element menu, double-click | coords, id, label, or `-d` grounding |
| `gesture-swipe` | `swipe --x --y --x2 --y2` | command bar (directional) | 0–1000 grid |
| `gesture-swipe` (momentum off) / `gesture-custom` | `drag --x --y --x2 --y2` | element menu (directional from selection) | slow press-move |
| `keyboard` (+ tap-to-focus) | `input --text` | element menu | tap-to-focus then append |
| `launch-app` | `activate-app --app-id` | element menu | |
| `button home` (+ `restart-app`) | `terminate-app --app-id` | element menu | `restart-app` = terminate + activate |
| `button home` | `home`, `background-app [--seconds]` | command bar + element menu | bare press vs sleep + reopen |
| `open-url` | `open-url --url` | element menu | schemes + universal links |
| describe + `gesture-tap` | `alert [--dismiss]` | element menu | taps the matching dialog button |
| `button back` (edge swipe on iOS) | `back` | command bar + element menu | |
| `gesture-swipe` | `scroll --direction [--amount]` | command bar + element menu | semantic; exact paths use swipe |
| `keyboard` | `keyboard [--action]` | command bar (dismiss) + element menu | |
| `describe` | `yoqa screen` | Refresh tree / cached select | cleaned 0–1000 tree |
| `screenshot` | `yoqa screenshot [path]` | element menu | primary read; live feed never persists |
| `wait` / `is` / `get` | `yoqa assert`, `sleep` | assert chips, wait control | no element-attribute reads in yoqa |
| `reinstall-app`, `rotate`, `shake`, `tv-remote`, `flow-*`, profiling | — | — | intentionally not exposed; use `argent run` directly |
...
6. Record taps / input / waits via the element menu (preferred locator + coordinates).
7. Command bar: **Scroll down** inserts `yoqa action scroll --direction down`; **Back** / **Home** / **Dismiss keyboard** insert their lines. Insert & Run each against the device.
8. Selector quality: click a blank scroll area or a deeplink cell — tap should be coords-only (no `--id 'scheme://…'`, no `--label 'XCUIElementType…'`); assert should prompt for text instead of inserting the type name.
9. App control / deeplink: select any element → **Selector Commands** → `activateApp` → insert & run; or use `openUrl` / `acceptAlert` as needed.
10. Capture evidence: **Selector Commands** → `screenshot` → Insert & Run.
11. Insert & Run a navigation tap → tree refreshes once → next click stays fast on the new cache.
12. Click **Save as test case** → name the case → **Create test case**; confirm Script tab has actions (`back`/`scroll`/`home`/`keyboard` steps warn and skip).
13. **Control-pick:** with Live control off, hold Control, move over a control the tree will not highlight, click — a point target and menu appear (`x,y` caption). Insert `tap (x,y)` (or double-tap / long-press / input). Save as test case → Script tab has `{ type: "tap", x, y }`. Replay on iOS and Android taps that screenshot point.
14. CLI parity: `yoqa action back`, `yoqa action scroll --direction down`, `yoqa action home`, `yoqa action keyboard` all print `ok <kind>`.

## Follow-ups

- All Commands catalog + View Docs links
- Agent decision kinds for `back` / `scroll` / `home` / `keyboard` (agent swipes today; see [agent CLI parity](../runs/agent-cli-parity.md))
- `scrollUntilVisible`, `copyTextFrom`, `extendedWaitUntil`
- CaseScript support for double / long-press / back / scroll / home / keyboard / activate-app / open-url / screenshot
...
- Optional JPEG/scale live frames if PNG poll proves slow on real devices

See also: [live feed (poll transport)](./manual-inspector-mjpeg-stream.md) (shipped).
