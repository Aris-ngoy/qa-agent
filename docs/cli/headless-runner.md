...
- `@yoqa/runner` npm package: `bun build --target bun` → `dist/index.js`, `yoqa-runner` bin stub that execs Bun. `@swmansion/argent` stays external (resolved at runtime from `node_modules/.bin`, then `PATH`).
...
Host Node/npm must be on `PATH`, plus the Argent CLI (`npm install -g @swmansion/argent`). Argent caches its iOS runner build between jobs. iOS still needs `macos-*` + Xcode; this is not a cloud device farm.
