...
Register it once with `yoqa builds create` to get a build ID (see [Builds](../references/builds.md)), then run the cases with `--build-id` per [Run Tests](../references/run.md). Reuse the same ID across runs instead of re-passing a path.
...
Run the saved case numbers with the run command in [Run Tests](../references/run.md). A `run_id` is returned — tell the user the run has started and share the ID. Inspect pass/fail per step with `yoqa runs get` (see [Run Tests](../references/run.md)).
...
- If the run fails with "Insufficient credits", tell the user to add credits or subscribe at https://yoqa.ai
