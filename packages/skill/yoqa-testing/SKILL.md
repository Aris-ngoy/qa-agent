...
1. **Check readiness** — `yoqa status` reports the runner, Argent runtime, AI provider, and active device
   in one shot. Any command other than `yoqa serve` starts the local runner automatically, so you rarely
   need to start it by hand. The device backend needs the Argent CLI (`npm install -g @swmansion/argent`) — visual-first: screenshot is the primary read, the accessibility tree is secondary.
...
- **If a command fails because the device or session is unhealthy** ("not reachable", "no active session",
  ...
  3. `yoqa runtime ensure` if the Argent runtime is missing (passes install consent)
