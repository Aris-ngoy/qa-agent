...
Device runs need a simulator or emulator and Argent (`yoqa doctor --fix` / `yoqa runtime ensure`). Description grounding and agent runs also need provider secrets. GitHub-hosted iOS needs `macos-*` + Xcode. This is not a cloud device farm — no runner cache is needed (Argent manages its own runner); `npm i -g @swmansion/argent` reinstalls fast.
...
yoqa screen --full                           # raw screen snapshot JSON
