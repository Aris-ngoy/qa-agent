...
<Note>
  Install the CLI from the desktop app: **Settings → CLI & Agents → Install CLI**. Restart your terminal afterward. For coding agents, also install the skill — see [CLI for agents](/guide/cli-for-agents).
</Note>
...
<Steps>
  ...
  <Step title="Install the CLI">
    Settings → **CLI & Agents** → **Install CLI**. Links `yoqa` into `~/.local/bin`.
  </Step>
