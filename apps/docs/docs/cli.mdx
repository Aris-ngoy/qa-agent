...
Install Bun and `@yoqa/cli`. The first `yoqa` command starts the runner; later steps reuse it. Host **Node/npm** must be on `PATH`, plus the agent-device CLI (`npm install -g agent-device@latest`).
...
Device runs need a simulator or emulator and agent-device (`yoqa doctor --fix` / `yoqa runtime ensure`). Description grounding and agent runs also need provider secrets. GitHub-hosted iOS needs `macos-*` + Xcode. This is not a cloud device farm — cache `~/.agent-device` between jobs to skip rebuilding the iOS runner.
...
yoqa screen --full                           # raw agent-device snapshot JSON
