...
The CLI is a thin HTTP client. In GitHub Actions (or any Node ≥20 job), install `@yoqa/cli`, start a runner on the same machine (desktop sidecar or monorepo `bun run runner`), then invoke commands:
...
```bash
yoqa doctor                                  # full diagnostics (runtime, drivers, host tools, leftover Appium)
