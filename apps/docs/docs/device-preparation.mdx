...
Yoqa drives devices through **Argent**. Install it once (`npm install -g @swmansion/argent`), get the toolchain healthy (`yoqa doctor`), then connect from the desktop UI or CLI.
...
3. For **physical devices**, enable Developer Mode and trust the host. Argent manages its own runner, so no runner signing is needed — the desktop **Settings → iOS** signing fields are legacy and no longer used.
...
<Tip>
  If runs fail before the first step, re-check `yoqa status` and `yoqa doctor` — most local issues are toolchain, not test wording.
</Tip>
