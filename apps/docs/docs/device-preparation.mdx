...
3. For **physical devices**, enable Developer Mode and trust the host. agent-device manages its own runner signing — for personal Apple IDs set `AGENT_DEVICE_IOS_BUNDLE_ID` to a unique reverse-DNS value (see [agent-device installation](https://oss.callstack.com/agent-device/docs/installation)).
