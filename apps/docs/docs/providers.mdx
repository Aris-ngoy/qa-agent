...
| **OpenCode** | Zen API key | Free / Paid model picker; the `opencode` CLI in `PATH` is not enough for vision — paste a Zen key to enable it |
...
- For OpenCode, vision requires a **Zen API key** — installing the `opencode` CLI alone will look connected but agent runs will fail. Paste the key in **Settings → Providers**, then pick a free vision-capable model unless you have paid Zen credits.
