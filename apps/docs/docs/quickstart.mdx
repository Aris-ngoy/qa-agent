...
<Steps>
  <Step title="Start YoQA">
    Download the Apple Silicon DMG from [GitHub Releases](https://github.com/Aris-ngoy/qa-agent/releases), drag `yoqa.app` to `/Applications`, and open it. The DMG is unsigned — right-click → **Open** the first time to bypass Gatekeeper. The bundled runner starts automatically on `http://127.0.0.1:7420`.
