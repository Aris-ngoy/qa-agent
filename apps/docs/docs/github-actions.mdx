...
`yoqa` talks to a **local runner** on the GitHub-hosted (or self-hosted) machine. This is not a cloud device farm: you still need a simulator or emulator, agent-device (`npm install -g agent-device@latest`, then `yoqa doctor --fix` / `yoqa runtime ensure`), and a catalog (apps/cases) on that host.
...
Cache `~/.agent-device` between jobs so the iOS runner is not rebuilt every run. Host **Node/npm** must stay on `PATH`.
