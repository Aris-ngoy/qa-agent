...
`yoqa` talks to a **local runner** on the GitHub-hosted (or self-hosted) machine. This is not a cloud device farm: you still need a simulator or emulator, Argent (`npm install -g @swmansion/argent`, then `yoqa doctor --fix` / `yoqa runtime ensure`), and a catalog (apps/cases) on that host.
...
No runner cache is needed between jobs — Argent manages its own runner; `npm i -g @swmansion/argent` reinstalls fast. Host **Node/npm** must stay on `PATH`.
