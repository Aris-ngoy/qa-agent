...
| GitHub Actions (your runner + sim/emulator) | [CLI CI example](/docs/cli) — `examples/expo-demo` |
...
You can also wire `yoqa` into GitHub Actions on `macos-*` with a simulator or emulator — see [CLI](/docs/cli). Cloud device farm and hosted Yoqa runners are still unshipped.
